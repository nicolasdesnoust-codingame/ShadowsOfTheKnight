...
public class Line {
    ...
    public Line getPerpendicular(Point point) {
        double perpendicularA = -1 / a;
        double perpendicularB = point.getY() - perpendicularA * point.getX();
        return new Line(perpendicularA, perpendicularB);
    }
	...
	public Point getPointBelow(Point origin, double distance) {
		double y = origin.getY() + 1;
		double x = ((y - b) / a);
		Point end = new Point(x, y);
		Vector vector = new Vector(origin, end);
