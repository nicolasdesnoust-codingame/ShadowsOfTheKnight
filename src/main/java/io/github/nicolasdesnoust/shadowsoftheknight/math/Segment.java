...
/*
        Vector ab = new Vector(this.p1, this.p2);
        Vector ac = new Vector(this.p1, point);
*/

        double crossProduct = (point.getY() - this.p1.getY()) * (this.p2.getX() - this.p1.getX())
            - (point.getX() - this.p1.getX()) * (this.p2.getY() - this.p1.getY());

        //compare versus epsilon for floating point values, or != 0 if using integers
        if (Math.round(Math.abs(crossProduct)) > epsilon) {
            return false;
        }

        double dotProduct = (point.getX() - this.p1.getX()) * (this.p2.getX() - this.p1.getX())
            + (point.getY() - this.p1.getY()) * (this.p2.getY() - this.p1.getY());

        if (dotProduct < 0) {
            return false;
        }

        double squaredlengthba = (this.p2.getX() - this.p1.getX()) * (this.p2.getX() - this.p1.getX())
